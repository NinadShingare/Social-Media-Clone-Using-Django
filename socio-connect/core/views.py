...
from .models import Profile, Post, LikePost, FollowersCount, Feedback
...
def feedback(request):
    ...
    if request.method=="POST":
        ...
        contact=Feedback(user=name, email=email, phone=phone, desc=desc, date=datetime.today())
        contact.save()
...
def changepass(request):
    ...
    #         return redirect('settings')
    #     else:
    #         messages.success(request, 'Password not changed')
    #     return render(request, 'setting.html', {'user_profile': user_profile})
    # else:
...
def changepass(request):
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            ...
            #user_profile.password = password2
            # user = User.objects.create_user( password=password)
            usr.save()
            #user_profile.save()
            messages.success(request, 'Password sucessfully changed')
